...
bool BIRAnnounceModule = 1;

class BIRConfig : public WorldScript
{
public:
    BIRConfig() : WorldScript("BIRConfig_conf") { }

    void OnBeforeConfigLoad(bool reload) override
    {
        if (!reload) {
            std::string conf_path = _CONF_DIR;
            std::string cfg_file = conf_path + "/mod_betteritemreload.conf";

#ifdef WIN32
            cfg_file = "mod_betteritemreload.conf";
#endif

            std::string cfg_def_file = cfg_file + ".dist";
            sConfigMgr->LoadMore(cfg_def_file.c_str());
            sConfigMgr->LoadMore(cfg_file.c_str());
            BIRAnnounceModule = sConfigMgr->GetBoolDefault("BIR.Announce", 1);
        }
    }
};

class BIRAnnounce : public PlayerScript
{

public:

    BIRAnnounce() : PlayerScript("BIRAnnounce") {}

    void OnLogin(Player* player)
    {
        // Announce Module
        if (BIRAnnounceModule)
        {
            ChatHandler(player->GetSession()).SendSysMessage("This server is running the |cff4CFF00BetterItemReload |rmodule.");
        }
    }
};
