...
--rand = 90 	-- debug scenes or set to static screen value
...
--					Scale	  X       Y       Z       O   	Model
	...
	-- The Crypt of Conan
	CreateModelFrame(0.05, 	3.92,	1.15,	0.00,	0.00, 	"Spells\\Archimonde_fire.m2")
	CreateModelFrame(0.03, 	7.50,	2.68,	0.00,	0.00, 	"World\\Generic\\passivedoodads\\summerfestival\\summerfest_candle_02.m2")
	CreateModelFrame(0.03, 	1.51,	2.09,	0.00,	0.00, 	"World\\Generic\\passivedoodads\\summerfestival\\summerfest_candle_02.m2")		
	
elseif rand >= 61 and rand <= 70 then
	-- The Dark Portal
	CreateModelFrame(0.40, 	5.60,	2.40,	0.00,	0.00, 	"Spells\\Creature_spellportal_green.m2")
	CreateModelFrame(0.70, 	5.60,	2.50,	0.00,	0.00, 	"Spells\\Druid_infectedwounds.m2")
	
	-- Pillar 
	CreateModelFrame(0.05, 	-9.00,	-9.00,	6.00, 	0.10, 	"Creature\\Eyeofkathune\\eyeofkathune.m2")
	
	-- Mage
	CreateModelFrame(1.5, 	3.28,	1.63,	0.00,	16.1, 	"Spells\\Conjureitemcast.m2")	
	
	-- Elf
	CreateModelFrame(1.00, 	6.90,	2.10,	0.00,	16.1, 	"Particles\\Lootfx.m2")		
	-- CreateModelFrame(0.40, 		6.10,		0.94,	0.00,	0.50, 	"Item\\Objectcomponents\\ammo\\arrowmagicflight_01.m2")

	-- Paladin
	CreateModelFrame(1.3, 	7.00,	2.10,	0.00,	5.00, 	"Spells\\Avengingwrath_state_chest.m2")	
...
elseif rand >=71 and rand <= 80 then
	...
	if (rand % 2 == 0) then
		...
		CreateModelFrame(1.00, 	4.25,	2.00,	0.00,	16.1, 	"World\\Kalimdor\\silithus\\passivedoodads\\ahnqirajglow\\quirajglow.m2")
		CreateModelFrame(0.20,  7.85, 	1.90,	0.00,	7.60, 	"World\\Expansion02\\doodads\\ulduar\\ul_brainparts_02.m2")	
	else
		-- Arcadia
		CreateModelFrame(0.20,  6.25, 	1.90,	0.00,	7.60, 	"World\\Expansion02\\doodads\\ulduar\\ul_brainparts_02.m2")
		CreateModelFrame(0.03,  2.40,	3.40,	0.00,	16.10, 	"World\\Expansion02\\doodads\\ulduar\\ul_planet_03.m2")
		CreateModelFrame(0.015, 6.50,	2.00,	0.00,	20.70, 	"World\\Expansion02\\doodads\\ulduar\\ul_planet_04.m2")
